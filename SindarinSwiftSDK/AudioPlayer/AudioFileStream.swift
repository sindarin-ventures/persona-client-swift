...
final class AudioFileStream: Sendable {
    ...
    func parseData(_ data: Data) {
        syncQueue.async { [weak self] in
            ...
            data.withUnsafeBytes { pointer in
                ...
                AudioFileStreamParseBytes(audioStreamID, UInt32(data.count), baseAddress, AudioFileStreamParseFlags(rawValue: 0))
            }
        }
    }
    ...
    func finishDataParsing() {
        syncQueue.async { [weak self] in
            guard let self, let audioStreamID else { return }
            AudioFileStreamParseBytes(audioStreamID, 0, nil, AudioFileStreamParseFlags(rawValue: 0))
